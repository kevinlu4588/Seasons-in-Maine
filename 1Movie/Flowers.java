...
public class Flowers  {
    ...
    float height;
    float width;
    final static float TWO_PI = (float)6.28;
    public Flowers(Movie movie) {
    ...
    void addConcentricFlowers(float x, float y, float maxRadius, float colorHue) {
      ...
      for (int i=1; i<6; i++) {
      	int randomGradient = (int) PApplet.constrain(movie.random(100), 0, 40);
        flowers.add(new Flower(x, y, i, zOscillator, maxRadius/10*(6-i), (int)(3+movie.random(3)), (int)(3+movie.random(9)), this,ZAcolor(255 - i*randomGradient - movie.frameCount%10,192-i*randomGradient + movie.frameCount%200,203-i*randomGradient -movie.frameCount%10), this) );
      }
    }
    ...
    class Flower {
      ...
      Flower(float x, float y, float zIndex, RandomOscillator zOscillator, float meanRadius, int petalCount, int pointsPerPetal, Flowers flowers, int clr, PApplet applet) {
        this.x = x;
